...
const content = fs.readFileSync('/app/applet/src/components/relatorio 2025 2026.content.txt', 'utf8');
...
lines.forEach(line => {
    ...
    let reason = "Outros";
    if (line.includes("BA TERIA  BAIXA") || line.includes("BAIXA")) reason = "Bateria Baixa";
    ...
    if (locStr.startsWith("3.ONCOLOGIA")) {
        ...
        floor = "Sub.Solo";
        sector = "ARM.INTERNA";
    } else if (locStr.includes("SS ARQUIVO RAIOX") || locStr.includes("ARQUIVO RAIOX")) {
        floor = "Sub.Solo";
...
events.forEach((val, i) => {
    val.id = "EV-R" + String(i + 1).padStart(3, '0');
});

fs.writeFileSync('/app/applet/parsed.json', JSON.stringify(events, null, 2));
