...
export function DataView({ eventsData, onEdit, onDelete, onClearAll, onClearHistory, auditLogs = [] }: DataViewProps) {
  return (
    <div className="space-y-8">
      ...
      <div className="bg-brand-light border border-blue-200 p-6 rounded-2xl flex flex-col md:flex-row gap-4 items-start">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-blue-100 flex items-center justify-between">
          ...
          {eventsData.length > 0 && (
            <button 
              onClick={onClearAll}
              className="px-3 py-1.5 text-sm font-medium text-blue-600 bg-brand-light hover:bg-blue-100 rounded-lg transition-colors flex items-center gap-1.5"
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-brand-light border-b border-blue-100 text-blue-500 text-sm">
            ...
            <tbody className="divide-y divide-blue-100">
              {eventsData.map((row) => {
                let priorityColor = 'bg-blue-100 text-brand-blue';
                  ...
                  <tr key={row.id} className="hover:bg-brand-light/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        row.osStatus === 'Aberta e Concluída' ? 'bg-blue-100 text-brand-blue' :
                        row.osStatus === 'Aberta e Não Concluída' ? 'bg-red-100 text-brand-red' :
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {onEdit && (
                          <button 
                            onClick={() => onEdit(row)}
                            className="p-2 text-blue-600 hover:bg-brand-light rounded-lg transition-colors"
                            title="Editar Evento"
                          >
