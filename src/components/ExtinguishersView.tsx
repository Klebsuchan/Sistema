    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-blue-100 p-6">
          <h3 className="text-sm font-semibold text-blue-800 mb-4">Extintores por Prédio</h3>
          <div className="h-64">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-blue-100 p-6">
          <h3 className="text-sm font-semibold text-blue-800 mb-4">Extintores por Tipo</h3>
          <div className="h-64">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden flex flex-col">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-blue-100">
              {filtered.map((ext) => (
                <tr key={ext.id} className="hover:bg-blue-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-blue-900">{ext.num_extintor}</div>
                    <div className="text-xs text-blue-500 mt-0.5">{ext.patrim}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-medium text-blue-800">{ext.predio || '-'}</div>
                    <div className="text-xs text-blue-500 mt-0.5">{ext.localizacao}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-medium text-blue-800">{ext.tipo}</div>
                    <div className="text-xs text-blue-500 mt-0.5">{ext.cap}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-blue-700"><span className="text-xs text-blue-400">Recarga:</span> {ext.prox_recarga}</div>
                    <div className="text-blue-700 mt-0.5"><span className="text-xs text-blue-400">Teste:</span> {ext.prox_teste}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      {ext.retiradoParaRecarga && <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-800">Recarga (Retirado)</span>}
                      {ext.coberturaNoLocal && <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-800">Cobertura Ativa</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1">
                      <span className={`w-2 h-2 rounded-full ${ext.checkLacre ? 'bg-blue-500' : 'bg-blue-500'}`} title="Lacre"></span>
                      <span className={`w-2 h-2 rounded-full ${ext.checkPressurizacao ? 'bg-blue-500' : 'bg-blue-500'}`} title="Pressurização"></span>
                      <span className={`w-2 h-2 rounded-full ${ext.checkAnel ? 'bg-blue-500' : 'bg-blue-500'}`} title="Anel"></span>
                      <span className={`w-2 h-2 rounded-full ${ext.checkPino ? 'bg-blue-500' : 'bg-blue-500'}`} title="Pino"></span>
                    </div>
                  </td>
        ...
        <div className="p-4 border-t border-blue-100 bg-blue-50 flex items-center justify-between text-xs text-blue-500">
          <div>Total filtrado: <span className="font-semibold text-blue-900">{filtered.length}</span> extintores</div>
